...
Double_t rootfitPDF(Double_t *x, Double_t *par)
{
    ...
    return ((TMath::Exp(par[1] * x[0])+ par[2]));
}
...
void Top_fit()
{
    ...
    RooRealVar tau("tau", "tau", -1.54, -5., 1.0);
    RooRealVar tau2("tau2", "tau2", -1.54, -10, -0.1);
    ...
    TF1 *f2 = new TF1("f2", rootfitPDF, 1., 25., 2);

    f2->SetParameter(0,-1.54);
    f2->SetParLimits(0, -5, -0.1);
