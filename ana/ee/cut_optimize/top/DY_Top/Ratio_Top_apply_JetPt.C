...
void for_bin_by_bin_sigleflavor_jet(int flavor, float hadronflavor, TH1D *h_fakerate, float nJetPt, double Weight, TH1D *h_tmp)
{
    if (hadronflavor == flavor)
    {
        if (nJetPt < 50)
        {
            double fakerate_weight = h_fakerate->GetBinContent(1);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 50 & nJetPt < 100)
        {
            double fakerate_weight = h_fakerate->GetBinContent(2);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 100 & nJetPt < 150)
        {
            double fakerate_weight = h_fakerate->GetBinContent(3);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 150 & nJetPt < 200)
        {
            double fakerate_weight = h_fakerate->GetBinContent(4);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 200 & nJetPt < 250)
        {
            double fakerate_weight = h_fakerate->GetBinContent(5);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 250 & nJetPt < 300)
        {
            double fakerate_weight = h_fakerate->GetBinContent(6);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 300 & nJetPt < 350)
        {
            double fakerate_weight = h_fakerate->GetBinContent(7);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 350 & nJetPt < 400)
        {
            double fakerate_weight = h_fakerate->GetBinContent(8);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 400 & nJetPt < 450)
        {
            double fakerate_weight = h_fakerate->GetBinContent(9);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 450 & nJetPt < 500)
        {
            double fakerate_weight = h_fakerate->GetBinContent(10);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 500 & nJetPt < 550)
        {
            double fakerate_weight = h_fakerate->GetBinContent(11);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 550 & nJetPt < 600)
        {
            double fakerate_weight = h_fakerate->GetBinContent(12);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 600 & nJetPt < 650)
        {
            double fakerate_weight = h_fakerate->GetBinContent(13);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 650 & nJetPt < 700)
        {
            double fakerate_weight = h_fakerate->GetBinContent(14);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 700 & nJetPt < 750)
        {
            double fakerate_weight = h_fakerate->GetBinContent(15);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 750 & nJetPt < 800)
        {
            double fakerate_weight = h_fakerate->GetBinContent(16);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 800 & nJetPt < 850)
        {
            double fakerate_weight = h_fakerate->GetBinContent(17);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 850 & nJetPt < 900)
        {
            double fakerate_weight = h_fakerate->GetBinContent(18);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 900 & nJetPt < 950)
        {
            double fakerate_weight = h_fakerate->GetBinContent(19);
            h_tmp->Fill(nJetPt, fakerate_weight * Weight);
        }
        else if (nJetPt > 950 & nJetPt < 1000)
        {
...
void for_bin_by_bin_doubleflavor_jet(int flavor1, int flavor2, float hadronflavor, TH1D *h_fakerate, float nJetPt, double Weight, TH1D *h_tmp)
{
    if (hadronflavor == flavor1 || hadronflavor == flavor2)
    {
        if (nJetPt < 50)
...
void Ratio_Top_apply_JetPt()
{
    ...
    TH1D *h_Top_bfake_rate = ((TH1D *)Topfile->Get("Top_bjetPT_fakeRate"));
    TH1D *h_Top_cfake_rate = ((TH1D *)Topfile->Get("Top_cjetPT_fakeRate"));
    TH1D *h_Top_lightfake_rate = ((TH1D *)Topfile->Get("Top_lightPT_fakeRate"));

    TH1D *h_DY_bfake_rate = ((TH1D *)DYfile->Get("HT_bjet_PtfakeRate"));
    TH1D *h_DY_cfake_rate = ((TH1D *)DYfile->Get("HT_cjet_PtfakeRate"));
    TH1D *h_DY_lightfake_rate = ((TH1D *)DYfile->Get("HT_light_PtfakeRate"));
    ...
    TH1D *h_Top_JetPT_CR = new TH1D("h_Top_JetPT_CR", "", 20, 0, 1000);
    h_Top_JetPT_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_CR->Sumw2();
    ...
    TH1D *h_Top_JetPT_heavy_CR = new TH1D("h_Top_JetPT_heavy_CR", "", 20, 0, 1000);
    h_Top_JetPT_heavy_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_heavy_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_heavy_CR->Sumw2();
    ...
    TH1D *h_Top_JetPT_bjet_CR = new TH1D("h_Top_JetPT_bjet_CR", "", 20, 0, 1000);
    h_Top_JetPT_bjet_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_bjet_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_bjet_CR->Sumw2();

    TH1D *h_Top_JetPT_bjet_bybin_CR = new TH1D("h_Top_JetPT_bjet_bybin_CR", "", 20, 0, 1000);
    h_Top_JetPT_bjet_bybin_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_bjet_bybin_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_bjet_bybin_CR->Sumw2();
    ...
    TH1D *h_Top_JetPT_cjet_CR = new TH1D("h_Top_JetPT_cjet_CR", "", 20, 0, 1000);
    h_Top_JetPT_cjet_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_cjet_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_cjet_CR->Sumw2();

    TH1D *h_Top_JetPT_cjet_bybin_CR = new TH1D("h_Top_JetPT_cjet_bybin_CR", "", 20, 0, 1000);
    h_Top_JetPT_cjet_bybin_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_cjet_bybin_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_cjet_bybin_CR->Sumw2();
    ...
    TH1D *h_Top_JetPT_twoflavor_cut = new TH1D("h_Top_JetPT_twoflavor_cut", "", 20, 0, 1000);
    h_Top_JetPT_twoflavor_cut->GetXaxis()->SetTitle("");
    h_Top_JetPT_twoflavor_cut->GetYaxis()->SetTitle("");
    h_Top_JetPT_twoflavor_cut->Sumw2();

    TH1D *h_Top_JetPT_light_CR = new TH1D("h_Top_JetPT_light_CR", "", 20, 0, 1000);
    h_Top_JetPT_light_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_light_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_light_CR->Sumw2();

    TH1D *h_Top_JetPT_light_bybin_CR = new TH1D("h_Top_JetPT_light_bybin_CR", "", 20, 0, 1000);
    h_Top_JetPT_light_bybin_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_light_bybin_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_light_bybin_CR->Sumw2();

    TH1D *h_Top_JetPT_twoflavorlight_bybin_CR = new TH1D("h_Top_JetPT_twoflavorlight_bybin_CR", "", 20, 0, 1000);
    h_Top_JetPT_twoflavorlight_bybin_CR->GetXaxis()->SetTitle("");
    h_Top_JetPT_twoflavorlight_bybin_CR->GetYaxis()->SetTitle("");
    h_Top_JetPT_twoflavorlight_bybin_CR->Sumw2();
    ...
    for (int evt = 0; evt < T_TTTo2L2Nu_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_TTTo2L2Nu_JetPT->size(); i++)
        {
            ...
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_TTTo2L2Nu_Jethadronflavor)[i], h_DY_bfake_rate, (*v_TTTo2L2Nu_JetPT)[i], TTTo2L2Nu_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_TTTo2L2Nu_Jethadronflavor)[i], h_DY_cfake_rate, (*v_TTTo2L2Nu_JetPT)[i], TTTo2L2Nu_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_TTTo2L2Nu_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTTo2L2Nu_JetPT)[i], TTTo2L2Nu_eventWeight, h_Top_JetPT_light_bybin_CR);
            //for_bin_by_bin_doubleflavor_jet(4, 0, (*v_TTTo2L2Nu_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTTo2L2Nu_JetPT)[i], TTTo2L2Nu_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            if ((*v_TTTo2L2Nu_alpha)[i] < 0.15)
            {
                ...
                for_doubleflavor_jet(4, 0, (*v_TTTo2L2Nu_Jethadronflavor)[i], (*v_TTTo2L2Nu_JetPT)[i], TTTo2L2Nu_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    } // End of TTTo2L2Nu
    ...
    for (int evt = 0; evt < T_ST_tW_top_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_ST_tW_top_JetPT->size(); i++)
        {
            ...
            // apply bin by bin fake rate
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_ST_tW_top_Jethadronflavor)[i], h_DY_bfake_rate, (*v_ST_tW_top_JetPT)[i], ST_tW_top_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_ST_tW_top_Jethadronflavor)[i], h_DY_cfake_rate, (*v_ST_tW_top_JetPT)[i], ST_tW_top_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_ST_tW_top_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_ST_tW_top_JetPT)[i], ST_tW_top_eventWeight, h_Top_JetPT_light_bybin_CR);
            //for_bin_by_bin_doubleflavor_jet(4, 0, (*v_ST_tW_top_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_ST_tW_top_JetPT)[i], ST_tW_top_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            if ((*v_ST_tW_top_alpha)[i] < 0.15)
            {
                ...
                for_doubleflavor_jet(4, 0, (*v_ST_tW_top_Jethadronflavor)[i], (*v_ST_tW_top_JetPT)[i], ST_tW_top_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    } // End of ST_tW_top
    ...
    for (int evt = 0; evt < T_ST_tW_antitop_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_ST_tW_antitop_JetPT->size(); i++)
        {
            ...
            // apply bin by bin fake rate
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_ST_tW_antitop_Jethadronflavor)[i], h_DY_bfake_rate, (*v_ST_tW_antitop_JetPT)[i], ST_tW_antitop_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_ST_tW_antitop_Jethadronflavor)[i], h_DY_cfake_rate, (*v_ST_tW_antitop_JetPT)[i], ST_tW_antitop_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_ST_tW_antitop_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_ST_tW_antitop_JetPT)[i], ST_tW_antitop_eventWeight, h_Top_JetPT_light_bybin_CR);
            //for_bin_by_bin_doubleflavor_jet(4, 0, (*v_ST_tW_antitop_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_ST_tW_antitop_JetPT)[i], ST_tW_antitop_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            ...
            if ((*v_ST_tW_antitop_alpha)[i] < 0.15)
            {
                ...
                for_doubleflavor_jet(4, 0, (*v_ST_tW_antitop_Jethadronflavor)[i], (*v_ST_tW_antitop_JetPT)[i], ST_tW_antitop_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    ...
    for (int evt = 0; evt < T_TTWJetsToLNu_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_TTWJetsToLNu_JetPT->size(); i++)
        {
            ...
            // apply bin by bin fake rate
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_TTWJetsToLNu_Jethadronflavor)[i], h_DY_bfake_rate, (*v_TTWJetsToLNu_JetPT)[i], TTWJetsToLNu_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_TTWJetsToLNu_Jethadronflavor)[i], h_DY_cfake_rate, (*v_TTWJetsToLNu_JetPT)[i], TTWJetsToLNu_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_TTWJetsToLNu_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTWJetsToLNu_JetPT)[i], TTWJetsToLNu_eventWeight, h_Top_JetPT_light_bybin_CR);
            //for_bin_by_bin_doubleflavor_jet(4, 0, (*v_TTWJetsToLNu_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTWJetsToLNu_JetPT)[i], TTWJetsToLNu_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            ...
            if ((*v_TTWJetsToLNu_alpha)[i] < 0.15)
            {
                ...
                for_doubleflavor_jet(4, 0, (*v_TTWJetsToLNu_Jethadronflavor)[i], (*v_TTWJetsToLNu_JetPT)[i], TTWJetsToLNu_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    } // End of TTWJetsToLNu
    ...
    for (int evt = 0; evt < T_TTWJetsToQQ_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_TTWJetsToQQ_JetPT->size(); i++)
        {
            ...
            // apply bin by bin fake rate
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_TTWJetsToQQ_Jethadronflavor)[i], h_DY_bfake_rate, (*v_TTWJetsToQQ_JetPT)[i], TTWJetsToQQ_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_TTWJetsToQQ_Jethadronflavor)[i], h_DY_cfake_rate, (*v_TTWJetsToQQ_JetPT)[i], TTWJetsToQQ_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_TTWJetsToQQ_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTWJetsToQQ_JetPT)[i], TTWJetsToQQ_eventWeight, h_Top_JetPT_light_bybin_CR);
            //for_bin_by_bin_doubleflavor_jet(4, 0, (*v_TTWJetsToQQ_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTWJetsToQQ_JetPT)[i], TTWJetsToQQ_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            ...
            if ((*v_TTWJetsToQQ_alpha)[i] < 0.15)
            {
                ...
                //for_doubleflavor_jet(4, 0, (*v_TTWJetsToQQ_Jethadronflavor)[i], (*v_TTWJetsToQQ_JetPT)[i], TTWJetsToQQ_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    } // End of TTWJetsToQQ
    ...
    for (int evt = 0; evt < T_TTZToQQ_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_TTZToQQ_JetPT->size(); i++)
        {
            ...
            // apply bin by bin fake rate
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_TTZToQQ_Jethadronflavor)[i], h_DY_bfake_rate, (*v_TTZToQQ_JetPT)[i], TTZToQQ_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_TTZToQQ_Jethadronflavor)[i], h_DY_cfake_rate, (*v_TTZToQQ_JetPT)[i], TTZToQQ_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_TTZToQQ_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTZToQQ_JetPT)[i], TTZToQQ_eventWeight, h_Top_JetPT_light_bybin_CR);
            //for_bin_by_bin_doubleflavor_jet(4, 0, (*v_TTZToQQ_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTZToQQ_JetPT)[i], TTZToQQ_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            ...
            if ((*v_TTZToQQ_alpha)[i] < 0.15)
            {
                ...
                for_doubleflavor_jet(4, 0, (*v_TTZToQQ_Jethadronflavor)[i], (*v_TTZToQQ_JetPT)[i], TTZToQQ_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    } // End of TTZToQQ
    ...
    for (int evt = 0; evt < T_TTZToLLNuNu_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_TTZToLLNuNu_JetPT->size(); i++)
        {
            ...
            // apply bin by bin fake rate
            //----------------------------
            for_bin_by_bin_sigleflavor_jet(5, (*v_TTZToLLNuNu_Jethadronflavor)[i], h_DY_bfake_rate, (*v_TTZToLLNuNu_JetPT)[i], TTZToLLNuNu_eventWeight, h_Top_JetPT_bjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(4, (*v_TTZToLLNuNu_Jethadronflavor)[i], h_DY_cfake_rate, (*v_TTZToLLNuNu_JetPT)[i], TTZToLLNuNu_eventWeight, h_Top_JetPT_cjet_bybin_CR);
            for_bin_by_bin_sigleflavor_jet(0, (*v_TTZToLLNuNu_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTZToLLNuNu_JetPT)[i], TTZToLLNuNu_eventWeight, h_Top_JetPT_light_bybin_CR);
            // for_bin_by_bin_doubleflavor_jet(4, 0, (*v_TTZToLLNuNu_Jethadronflavor)[i], h_DY_lightfake_rate, (*v_TTZToLLNuNu_JetPT)[i], TTZToLLNuNu_eventWeight, h_Top_JetPT_twoflavorlight_bybin_CR);
            ...
            if ((*v_TTZToLLNuNu_alpha)[i] < 0.15)
            {
                ...
                for_doubleflavor_jet(4, 0, (*v_TTZToLLNuNu_Jethadronflavor)[i], (*v_TTZToLLNuNu_JetPT)[i], TTZToLLNuNu_eventWeight, h_Top_JetPT_twoflavor_cut);
            } // alpha cut
        }
    } // End of TTZToLLNuNu
    ...
    h_Top_JetPT_light_bybin_CR->GetYaxis()->SetTitle("nJet");
    h_Top_JetPT_light_cut->GetXaxis()->SetTitle("Jet PT");
    h_Top_JetPT_light_bybin_CR->GetXaxis()->SetTitle("Jet PT");
    ...
    h_Top_JetPT_cjet_bybin_CR->GetYaxis()->SetTitle("nJet");
    h_Top_JetPT_cjet_cut->GetXaxis()->SetTitle("Jet Pt");
    h_Top_JetPT_cjet_bybin_CR->GetXaxis()->SetTitle("Jet Pt");
    ...
    h_Top_JetPT_light_bybin_CR->Draw("hist e ");
    //h_Top_JetPT_bjet_bybin_CR->Draw("hist e same");
    h_Top_JetPT_light_cut->Draw("hist e same");
    ...
    */
    ...
    gPad->SetBottomMargin(0.2);
    gPad->SetTickx();
    TH1D *Ratioresult = new TH1D("Ratioresult", "",20,0,1000);
    Ratioresult->Sumw2();
    int nBins = h_Top_JetPT_bjet_cut->GetNbinsX();
    for (int i = 1; i <= nBins; i++)
    {
        double fit_content = h_Top_JetPT_bjet_CR->GetBinContent(i);
        double MC_content = h_Top_JetPT_bjet_cut->GetBinContent(i);
    ...
    */
    ...
    l0->Draw();
    c1->cd(2);
    h_Top_JetPT_cjet_bybin_CR->Draw("hist e ");
    h_Top_JetPT_cjet_cut->Draw("hist e same");
    c1->cd(3);
    h_Top_JetPT_light_bybin_CR->Draw("hist e ");
    h_Top_JetPT_light_cut->Draw("hist e same");
}
