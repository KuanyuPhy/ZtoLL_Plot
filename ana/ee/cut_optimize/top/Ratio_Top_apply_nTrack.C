...
{
    double par0 = -0.31213349853753525;
    double par1 = -0.3369597634649318;
    double par2 = 0.01007077697094194;
    double par3 = -0.003960699254309169;
    double par4 = 0.0291339908830591;
    Double_t weight = ((TMath::Exp(par0 + par1 * x + par2 * x * x)) + par3 * x + par4);
    return weight;
}
Double_t cjet_background(Double_t x)
{
    double par0 = -0.3248291488513345;
    double par1 = -0.7911177718871826;
    double par2 = -0.0012678123101242413;
    double par3 = 0.01978378986710919;
    Double_t weight = ((TMath::Exp(par0 + par1 * x)) + par2 * x + par3);
    return weight;
}
Double_t lightjet_background(float x)
{
    double par0 = -0.1801414778747165;
    double par1 = -0.5934543801756226;
    double par2 = -0.010654744833624658;
    double par3 = -9.762305271053515e-05;
    double par4 = 0.0025466629971345888;
    Double_t weight = ((TMath::Exp(par0 + par1 * x + par2 * x * x)) + par3 * x + par4);
    return weight;
}
...
void Ratio_Top_apply_nTrack()
{
    ...
    TH1D *h_Top_nTracks_CR = new TH1D("h_Top_nTracks_CR", "", 50, 0, 50);
    ...
    TH1D *h_Top_nTracks_heavy_CR = new TH1D("h_Top_nTracks_heavy_CR", "", 50, 0, 50);
    ...
    TH1D *h_Top_nTracks_cjet_CR = new TH1D("h_Top_nTracks_cjet_CR", "", 50, 0, 50);
    h_Top_nTracks_cjet_CR->GetXaxis()->SetTitle("");
    h_Top_nTracks_cjet_CR->GetYaxis()->SetTitle("");
    h_Top_nTracks_cjet_CR->Sumw2();

    TH1D *h_Top_nTracks_cjet_bybin_CR = new TH1D("h_Top_nTracks_cjet_bybin_CR", "", 50, 0, 50);
    ...
    TH1D *h_Top_nTracks_light_CR = new TH1D("h_Top_nTracks_light_CR", "", 50, 0, 50);
    h_Top_nTracks_light_CR->GetXaxis()->SetTitle("");
    h_Top_nTracks_light_CR->GetYaxis()->SetTitle("");
    h_Top_nTracks_light_CR->Sumw2();

    TH1D *h_Top_nTracks_light_bybin_CR = new TH1D("h_Top_nTracks_light_bybin_CR", "", 50, 0, 50);
    ...
    for (int evt = 0; evt < T_TTWJetsToQQ_tree->GetEntries(); evt++)
    {
        ...
        for (size_t i = 0; i < v_TTWJetsToQQ_nTrack->size(); i++)
        {
            ...
            for_signalflavor_jet(5, (*v_TTWJetsToQQ_Jethadronflavor)[i], (*v_TTWJetsToQQ_nTrack)[i], bWeightbybin, h_Top_nTracks_bjet_bybin_CR);
            for_signalflavor_jet(4, (*v_TTWJetsToQQ_Jethadronflavor)[i], (*v_TTWJetsToQQ_nTrack)[i], cWeightbybin, h_Top_nTracks_cjet_bybin_CR);
            for_signalflavor_jet(0, (*v_TTWJetsToQQ_Jethadronflavor)[i], (*v_TTWJetsToQQ_nTrack)[i], lightWeightbybin, h_Top_nTracks_light_bybin_CR);
            //----------------------------
            // apply fit fake rate curve
            //----------------------------
    ...
    //h_Top_nTracks_bjet_cut->SetLabelSize(0);
    //h_Top_nTracks_cjet_cut->SetLabelSize(0);
    //h_Top_nTracks_light_cut->SetLabelSize(0);
    h_Top_nTracks_cjet_cut->GetXaxis()->SetRangeUser(0, cjetbinmax+1);
    h_Top_nTracks_cjet_cut->Draw("hist e ");
    h_Top_nTracks_cjet_bybin_CR->Draw("hist e same ");
    //h_Top_nTracks_light_CR->Draw("hist e same");
