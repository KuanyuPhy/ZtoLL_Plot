...
public:
  ...
  virtual std::list<Double_t>* binBoundaries(RooAbsRealLValue& obs, Double_t xlow, Double_t xhi)
  const { return TRooAbsH1Fillable::binBoundaries(obs,xlow,xhi); }
  virtual Bool_t isBinnedDistribution(const RooArgSet& obs) const { return
  TRooAbsH1Fillable::isBinnedDistribution(obs); }
